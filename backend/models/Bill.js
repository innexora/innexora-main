...
billSchema.pre("save", function (next) {
  ...
  // Enhanced status management - handle all scenarios including when paid bills get new items
  if (this.balanceAmount <= 0 && this.totalAmount > 0) {
    // Only finalize if guest has checked out, otherwise keep as paid
    if (this.isGuestCheckedOut) {
    ...
    this.status = "active"; // Empty bill
  }

  // If bill was paid but now has positive balance due to new items, reset to appropriate status
...
billSchema.statics.createBillForGuest = async function (
  guestId,
  guestName,
  roomId,
  roomNumber,
  ...
  try {
    ...
    console.log(
      `✅ Created bill ${bill.billNumber} for guest ${guestName} - Room ${roomNumber} - Amount: ₹${totalAmount}`
    );
    return bill;
  } catch (error) {
...
billSchema.statics.markGuestCheckedOut = async function (guestId) {
  try {
    ...
    console.error("Error marking bills as checked out:", error);
    throw error;
  }
};
...
billSchema.statics.getActiveBills = async function () {
  try {
    ...
    const activeBills = bills.filter(
      (bill) => bill.guest && bill.guest.status === "checked_in"
    );
...
billSchema.statics.getFinalizedBills = async function () {
  try {
    ...
    const bills = await this.find({
      $or: [{ status: "finalized" }, { isGuestCheckedOut: true }],
    })
      .populate("guest", "name phone email status")
      .populate("room", "number type floor");

    // Filter to only show bills for guests who have checked out
    const finalizedBills = bills.filter(
      (bill) => bill.guest && bill.guest.status === "checked_out"
    );
...
billSchema.statics.addOrderToBill = async function (guestId, order) {
  try {
    ...
    await bill.save();
    console.log(
      `✅ Added order ${order.orderNumber} to bill ${bill.billNumber}`
    );
    return bill;
  } catch (error) {
...
billSchema.statics.recordPayment = async function (guestId, paymentData) {
  try {
    ...
    await bill.save();
    console.log(
      `✅ Recorded payment of ₹${paymentData.amount} for bill ${bill.billNumber}`
    );
    return bill;
  } catch (error) {
